...
function deleteProfile(id){if(db.profiles.length<=2)return alert('最低2プロフィール必要です。');const p=profile(id);if(!confirm(`${p?.name||'このプロフィール'}を削除しますか？ 学習履歴も削除されます。`))return;db.profiles=db.profiles.filter(x=>x.id!==id);save()}
...
$('#new-profile').onclick=()=>openProfile();
